/*
 * $Id: msqltcl.c,v 1.99 1996/09/27 20:53:09 hs Rel $
 *
...
static int
msql_posix_confl (char* syscall)
{
  char buf[MSQL_SMALL_SIZE];
  char* msg = Tcl_ErrnoMsg (errno);
...
static void
msql_clear_msg(Tcl_Interp* interp)
{
 ...
 * 'check' may be a pointer to a check function, or NULL.
...
static int
msql_prologue (Tcl_Interp* interp, int argc, char** argv, int req_args,
	       int (*check) (), char* usage_msg)
{
 ...
 * msql_version
 *
 * RETURNS the msqltcl version number.
 *
 */
...
msql_version ()
{
  static char buf[] = "$Revision: 1.99 $";
  char* ch1;
  char* ch2;

  /* Find first digit. */
  for (ch1 = buf; *ch1 < '0' || '9' < *ch1; ++ch1);
  /* Move digits and dots to the beginning of the string. */
  for (ch2 = buf; ('0' <= *ch1 && *ch1 <= '9') || *ch1 == '.';
       *ch2++ = *ch1++);
...
static int
msql_colinfo (Tcl_Interp* interp, m_field* fld, char* keyw)
{
  char buf[MSQL_SMALL_SIZE];
  ...
  switch (idx)
    ...
    case MSQL_COL_PRIMKEY_K:
      sprintf (buf, "%c", (IS_UNIQUE(fld->flags))?'1':'0') ;
      res = buf ;
      break ;
...
void
Msqltcl_Kill (ClientData clientData)
{
  int i ;
...
int
Msqltcl_Init (Tcl_Interp* interp)
{
  int i;
  ...
  MsqlTclVersion = msql_version ();

  /* Link the version variable read-only. */
  (void)sprintf (nbuf, "%s(%s)", MsqlStatusArr, MSQL_STATUS_VERSION);
  (void)Tcl_LinkVar (interp, nbuf, (char*)&MsqlTclVersion,
  ...
  /* Everything seems OK so far; time to make ourselves noticed. */
  return Tcl_PkgProvide (interp, "msqltcl", MsqlTclVersion);
}
...
int
Msqltcl_Connect (ClientData clientData, Tcl_Interp* interp,
		 int argc, char** argv)
{
  int        hand = -1;
  int        i;
...
Msqltcl_Use (ClientData clientData, Tcl_Interp* interp,
	     int argc, char** argv)
{
  int hand;
...
Msqltcl_Sel (ClientData clientData, Tcl_Interp* interp,
	     int argc, char** argv)
{
  int     hand;
...
int
Msqltcl_Exec (ClientData clientData, Tcl_Interp* interp,
	      int argc, char** argv)
{
  int     hand;
...
Msqltcl_Next (ClientData clientData, Tcl_Interp* interp,
	      int argc, char** argv)
{
  int hand;
  int idx ;
...
Msqltcl_Seek (ClientData clientData, Tcl_Interp* interp,
	      int argc, char** argv)
{
    int hand;
    int res;
...
int
Msqltcl_Map (ClientData clientData, Tcl_Interp* interp,
	     int argc, char** argv)
{
  int code ;
  int count ;
...
int
Msqltcl_Info (ClientData clientData, Tcl_Interp* interp,
		 int argc, char** argv)
{
  char buf[MSQL_BUFF_SIZE];
  int count ;
...
int
Msqltcl_Result (ClientData clientData, Tcl_Interp* interp,
		int argc, char** argv)
{
  char buf[MSQL_BUFF_SIZE];
  int count ;
...
Msqltcl_Col (ClientData clientData, Tcl_Interp* interp,
	     int argc, char** argv)
{
  char buf[MSQL_BUFF_SIZE];
  int coln ;
...
Msqltcl_State (ClientData clientData, Tcl_Interp* interp,
	       int argc, char** argv)
{
  int hi;
  char* hp ;
...
Msqltcl_Close (ClientData clientData, Tcl_Interp* interp,
	       int argc, char** argv)
{
  int     hand;
